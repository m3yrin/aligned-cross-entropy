...
from ..utils.loss import sequence_ctc_loss_with_logits, sequence_axe_loss_with_logits
...
class LatentAignmentCTC(Model):
    ...
    # TODO: too cheap, O(n * m), need pallalel processing
    ...
    def _get_loss(self, output_dict: Dict[str, torch.Tensor], target_tokens: Dict[str, torch.LongTensor]) -> torch.Tensor:
        ...
        if self.loss_type == "axe":
            return sequence_axe_loss_with_logits(alignment_logits, source_mask_upsampled, targets, target_mask, self._blank_index, self.label_smoothing)
